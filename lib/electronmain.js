var myProductName = "electronLand", myVersion = "0.5.6";   
...
var config = {
	...
	flPreventHardwareAcceleration: false //8/27/22 by DW
	};
...
function readUserDataFile (path, config, callback) {
	var f = getLocalFilePath (path);
	console.log ("readUserDataFile: f == " + f); //7/29/23 by DW
	utils.sureFilePath (f, function () {
...
function electronStartup () {
	...
	function tinyHttpServer (httpRequest, httpResponse) {
		...
		if (parsedUrl.search != null) { //3/4/23 by DW
	...
	if (config.flPreventHardwareAcceleration) { //8/27/22 by DW
...
function init (configParam, callback) {
	function checkForRelocatedUserConfigFile (callback) { //7/29/23 by DW
		fs.readFile (config.userDataFolder + fnameConfig, function (err, jsontext) {
			if (!err) {
				try {
					const jstruct = JSON.parse (jsontext);
					if (jstruct.userDataFolder !== undefined) { 
						config.userDataFolder = jstruct.userDataFolder;
						console.log ("checkForRelocatedUserConfigFile: config.userDataFolder == " + config.userDataFolder);
						}
					}
				catch (err) {
					}
	...
	console.log ("electronLand.init: config.userDataFolder == " + config.userDataFolder); //7/29/23 by DW
		...
		readConfig (config.appDirname + "/" + fnameConfig, config, function () {
			readUserDataFile (fnameConfig, config, function () {
				readUserDataFile (fnameStats, stats, function () {
					checkForRelocatedUserConfigFile (function () { //7/29/23 by DW
						stats.ctStarts++;
