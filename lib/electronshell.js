...
function shellSetupIpcHandlers () {
	...
	ipcRenderer.on ("webSocketCallback", function (event, s) {
		webSocketProcessMessage (s);
		});
	ipcRenderer.on ("statsChangedCallback", function (event, jsontext) {
		dashboardStatsChanged (jsontext);
		});
...
function shellOpenFileDialog (callback) {
	var options = {
		...
		properties: ["createDirectory"] //6/12/17 by DW
		};
